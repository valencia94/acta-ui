...
async function testDynamoComponents() {
  ...
  try {
    ...
    const initialCheck = await page.evaluate(() => {
      return {
        hasProjectElements: !!document.querySelector(
          '[data-testid*="project"], .project, [class*="project"], [id*="project"]',
        ),
        hasDynamoElements: !!document.querySelector(
          '[data-testid*="dynamo"], .dynamo, [class*="dynamo"], [id*="dynamo"]',
        ),
        hasPMElements: !!document.querySelector(
          '[data-testid*="pm"], .pm, [class*="pm"], [id*="pm"]',
        ),
        hasTableElements: !!document.querySelector(
          'table, [role="table"], .table, [class*="table"]',
        ),
        hasLoadingElements: !!document.querySelector(
          '[data-testid*="loading"], .loading, [class*="loading"]',
        ),
        hasErrorElements: !!document.querySelector(
    ...
    if (isOnLogin) {
      ...
      const loginPageCode = await page.evaluate(() => {
        ...
        return {
          hasDashboardRef:
            scripts.includes("Dashboard") || scripts.includes("dashboard"),
          hasDynamoRef:
            scripts.includes("Dynamo") || scripts.includes("dynamo"),
          hasPMRef:
            scripts.includes("PM") || scripts.includes("ProjectManager"),
    ...
    const windowCheck = await page.evaluate(() => {
      ...
      components.forEach((comp) => {
        results[comp] = {
          ...
          inObjectKeys: Object.keys(window).some((key) =>
            key.includes(comp.toLowerCase()),
          ),
        };
      });
