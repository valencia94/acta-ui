...
const criticalPages = ["Dashboard", "AdminDashboard", "Login"];
...
const criticalUtils = ["fetchWrapper", "getAuthToken", "backendDiagnostic"];
...
function checkBundleContents() {
  ...
  console.log(
    `✅ Found main bundle: ${jsFiles[0]} (${(fs.statSync(mainBundle).size / 1024 / 1024).toFixed(2)}MB)`,
  );
  ...
  if (fs.existsSync(awsConfigFile)) {
    ...
    console.log(
      `  ✅ aws-exports.js: Found (${fs.statSync(awsConfigFile).size} bytes)`,
    );
    ...
    const awsChecks = [
      ...
      {
        prop: "aws_user_pools_web_client_id",
        expected: "dshos5iou44tuach7ta3ici5m",
      },
      {
        prop: "endpoint",
        expected: "q2b9avfwv5.execute-api.us-east-2.amazonaws.com",
      },
    ];
  ...
  console.log("=".repeat(50));
  console.log(
    `Components: ${criticalComponents.length - componentIssues}/${criticalComponents.length} found`,
  );
  console.log(
    `Pages: ${criticalPages.length - pageIssues}/${criticalPages.length} found`,
  );
  console.log(
    `API Functions: ${criticalApiFunctions.length - apiIssues}/${criticalApiFunctions.length} found`,
  );
  console.log(
    `Utilities: ${criticalUtils.length - utilIssues}/${criticalUtils.length} found`,
  );
...
async function main() {
  ...
  if (sourceFilesOk && viteConfigOk && bundleOk) {
    console.log(
      "🎉 SUCCESS: All components verified and ready for deployment!",
    );
    process.exit(0);
  } else {
