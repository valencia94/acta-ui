...
const CONFIG = {
  ...
  userPoolArn: 'arn:aws:cognito-idp:us-east-2:703671891952:userpool/us-east-2_FyHLtOhiY',
...
const PROTECTED_ENDPOINTS = [
  ...
  { path: '/check-document/{projectId}', resourceId: '0rpaw2', methods: ['GET'] },
...
async function fixApiGatewayConfiguration() {
  ...
  const cognitoAuthorizer = authorizers.items.find(auth => 
    auth.name === 'CognitoUserPoolAuthorizer' || auth.type === 'COGNITO_USER_POOLS'
  );
  ...
  if (!cognitoAuthorizer) {
    ...
    console.log(`✅ Created Cognito Authorizer: ${newAuthorizer.name} (ID: ${newAuthorizer.id})`);
    cognitoAuthorizer.id = newAuthorizer.id;
  } else {
    console.log(`✅ Found Cognito Authorizer: ${cognitoAuthorizer.name} (ID: ${cognitoAuthorizer.id})`);
  }
  ...
  for (const endpoint of PROTECTED_ENDPOINTS) {
    ...
    for (const method of endpoint.methods) {
      ...
      if (updateResult) {
        console.log(`      ✅ ${method} method updated with Cognito authorization`);
      }
    }
  }
  ...
  if (deployResult) {
    ...
    console.log(`✅ API deployed successfully! Deployment ID: ${deployment.id}`);
  }
