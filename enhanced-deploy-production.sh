...
# Check for Cognito domain in build
if grep -r "us-east-2-fyhltohiy\.auth\.us-east-2\.amazoncognito\.com" $BUILD_DIR/assets/*.js 2>/dev/null; then
  ...
  echo "⚠️ This indicates aws-exports.js was not properly included in the build!"
  exit 1
fi
...
# Upload JavaScript files with correct content type and long cache headers
...
# Upload CSS files with correct content type and long cache headers
aws s3 sync $BUILD_DIR/ s3://$S3_BUCKET/ \
    --region $AWS_REGION \
    --cache-control "public, max-age=31536000" \
...
# Upload PNG files with correct content type and long cache headers
aws s3 sync $BUILD_DIR/ s3://$S3_BUCKET/ \
    --region $AWS_REGION \
    --cache-control "public, max-age=31536000" \
...
# Upload other static assets with long cache headers
aws s3 sync $BUILD_DIR/ s3://$S3_BUCKET/ \
    --region $AWS_REGION \
    --delete \
    --cache-control "public, max-age=31536000" \
