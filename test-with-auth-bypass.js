  ...
  try {
    ...
    const dashboardState = await page.evaluate(() => {
      return {
        ...
        componentClasses: Array.from(document.querySelectorAll('[class*="component"], [class*="Component"]'))
          .map(el => el.className).slice(0, 10),
        dataTestIds: Array.from(document.querySelectorAll('[data-testid]'))
          .map(el => el.getAttribute('data-testid')).slice(0, 10)
      };
    });
    ...
    Object.entries(dashboardState.hasElements).forEach(([element, value]) => {
      const display = typeof value === 'number' ? `${value} found` : (value ? '✅' : '❌');
    ...
    const errorCheck = await page.evaluate(() => {
      ...
      return {
        ...
        specificErrors: Array.from(document.querySelectorAll('.error, [class*="error"], .alert-error'))
    ...
    Object.entries(errorCheck).forEach(([check, result]) => {
      if (Array.isArray(result)) {
        console.log(`  ${check}: ${result.length > 0 ? result.join(', ') : 'None'}`);
      } else {
    ...
    const finalCheck = await page.evaluate(() => {
      return {
        ...
        uniqueClasses: [...new Set(Array.from(document.querySelectorAll('[class]'))
          .map(el => el.className.split(' ')).flat())].filter(c => c).slice(0, 20)
      };
    });
