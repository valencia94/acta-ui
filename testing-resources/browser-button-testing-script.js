...
// Run this in the browser console on https://d13zx5u8i7fdt7.cloudfront.net
...
function checkAuthStatus() {
    ...
    const authChecks = [
        ...
        { key: 'CognitoIdentityServiceProvider', storage: 'localStorage', partial: true },
    ...
    authChecks.forEach(check => {
        const storage = check.storage === 'localStorage' ? localStorage : sessionStorage;
        if (check.partial) {
            // Check for keys that start with the pattern
            const keys = Object.keys(storage).filter(key => key.includes(check.key));
            authStatus[check.key] = keys.length > 0 ? keys : null;
        } else {
            authStatus[check.key] = storage.getItem(check.key) ? 'Present' : 'Not found';
        }
    });
...
function discoverButtons() {
    ...
    console.log(`Found ${allButtons.length} potential action buttons:`, allButtons);
...
function testButton(buttonInfo, expectedEndpoint) {
    ...
    buttonInfo.element.addEventListener('click', function(e) {
...
function generateTestReport(authStatus, buttons, requests) {
    ...
    const report = {
        ...
        apiRequests: requests.filter(r => 
            r.url && (r.url.includes('api.acta.ikusii.com') || r.url.includes('/api/'))
        ),
        buttons: buttons,
...
console.log('✅ Test suite loaded! Run actaTestSuite.runButtonTests() to start comprehensive testing.');
