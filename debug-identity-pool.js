...
async function debugIdentityPool() {
    ...
    try {
        ...
        const poolDetails = await cognitoIdentity.describeIdentityPool({
            IdentityPoolId: IDENTITY_POOL_ID
        }).promise();
        
        console.log('\n📋 Identity Pool Details:');
        console.log('- Identity Pool Name:', poolDetails.IdentityPoolName);
        console.log('- Allow Unauthenticated:', poolDetails.AllowUnauthenticatedIdentities);
        console.log('- Allow Classic Flow:', poolDetails.AllowClassicFlow);
        console.log('- Cognito Identity Providers:', JSON.stringify(poolDetails.CognitoIdentityProviders, null, 2));
        
        // Get identity pool roles
        const rolesResult = await cognitoIdentity.getIdentityPoolRoles({
            IdentityPoolId: IDENTITY_POOL_ID
        }).promise();
        
        console.log('\n🔐 Identity Pool Roles:');
        console.log('- Roles:', JSON.stringify(rolesResult.Roles, null, 2));
        console.log('- Role Mappings:', JSON.stringify(rolesResult.RoleMappings, null, 2));
        ...
        try {
            const guestCredentials = await cognitoIdentity.getId({
                IdentityPoolId: IDENTITY_POOL_ID
            }).promise();
