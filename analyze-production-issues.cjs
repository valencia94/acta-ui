...
async function analyzeIssues() {
  ...
  await makeAPICall('/pm-manager/all-projects', '👥 PM All Projects (should require auth)');
  await makeAPICall(`/pm-manager/test@example.com`, '📧 PM Email Projects (should require auth)');
  
  // Test document-related endpoints
  await makeAPICall(`/check-document/${TEST_PROJECT_ID}`, `📄 Document Check for ${TEST_PROJECT_ID}`);
  await makeAPICall(`/download-acta/${TEST_PROJECT_ID}?format=pdf`, `📥 Download PDF for ${TEST_PROJECT_ID}`);
  
  // Test project summary
  await makeAPICall(`/project-summary/${TEST_PROJECT_ID}`, `📋 Project Summary for ${TEST_PROJECT_ID}`);
  ...
  console.log('   → Frontend may not be extracting user email from auth session');
