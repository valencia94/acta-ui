...
async function fetcher(input, init = {}) {
  ...
  if (token) {
    ...
    console.warn(
      "❌ No auth token available - API call will likely fail with 401",
    );
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        ...
        errorMessage +=
          " (Authentication expired - please retry after page refresh)";
      } else if (res.status === 403) {
        errorMessage +=
          " (Insufficient permissions - check role/claims in token)";
    ...
    if (isLoginPage) {
      ...
      await page.evaluate(() => {
        ...
        try {
          ...
          window.testAuthFlowResults.errors.push(
            "Error checking aws-exports: " + e.message,
          );
        }
        ...
        try {
          ...
          window.testAuthFlowResults.errors.push(
            "Error checking Amplify: " + e.message,
          );
        }
        ...
        try {
          ...
          if (typeof fetchAuthSession === "function") {
            ...
            window.testAuthFlowResults.errors.push(
              "fetchAuthSession not available",
            );
          }
        } catch (e) {
          console.log("❌ Error checking fetchAuthSession:", e);
          window.testAuthFlowResults.errors.push(
            "Error checking fetchAuthSession: " + e.message,
          );
        }
      });
      ...
      console.log(
        "AWS Exports Present:",
        testResults.awsExportsPresent ? "✅ YES" : "❌ NO",
      );
      console.log(
        "Amplify Present:",
        testResults.amplifyPresent ? "✅ YES" : "❌ NO",
      );
      console.log(
        "fetchAuthSession Present:",
        testResults.fetchAuthSessionPresent ? "✅ YES" : "❌ NO",
      );
