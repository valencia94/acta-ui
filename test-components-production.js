...
async function testComponentsInProduction() {
  ...
  page.on("console", (msg) => {
    const text = msg.text();
    if (
      text.includes("aws-exports") ||
      text.includes("Critical functions found")
    ) {
      console.log("ℹ️", text);
    }
  });
  ...
  try {
    ...
    const componentCheck = await page.evaluate(() => {
      ...
      results.reactComponents = {
        reactPresent:
          allScripts.includes("React") || allScripts.includes("__react"),
        routerPresent:
          allScripts.includes("Router") || allScripts.includes("navigate"),
        amplifyPresent:
          allScripts.includes("Amplify") || allScripts.includes("aws-amplify"),
        framerMotionPresent:
          allScripts.includes("motion") || allScripts.includes("framer"),
      };
    ...
    Object.entries(componentCheck.uiElements).forEach(([key, value]) => {
      console.log(
        `  ${value ? "✅" : "❌"} ${key}: ${value ? "Present" : "Missing"}`,
      );
    });
    ...
    Object.entries(componentCheck.criticalFunctions).forEach(([key, value]) => {
      console.log(
        `  ${value ? "✅" : "❌"} ${key}: ${value ? "Available" : "Missing"}`,
      );
    });
    ...
    console.log(
      `  ${awsCheck.awsmobileExists ? "✅" : "❌"} awsmobile object: ${awsCheck.awsmobileExists ? "Present" : "Missing"}`,
    );
    ...
    if (isLoginPage) {
      ...
      try {
        ...
        console.log(
          `  ${isEnabled ? "✅" : "⚠️"} Sign In button: ${isEnabled ? "Enabled" : "Disabled"}`,
        );
      } catch (error) {
    ...
    // Summary
    const totalUIElements = Object.values(componentCheck.uiElements).filter(
      Boolean,
    ).length;
    const totalFunctions = Object.values(
      componentCheck.criticalFunctions,
    ).filter(Boolean).length;
    const totalReactComponents = Object.values(
      componentCheck.reactComponents,
    ).filter(Boolean).length;

    console.log("\n📊 SUMMARY:");
    console.log("=".repeat(50));
    console.log(
      `UI Elements: ${totalUIElements}/${Object.keys(componentCheck.uiElements).length} working`,
    );
    console.log(
      `Critical Functions: ${totalFunctions}/${Object.keys(componentCheck.criticalFunctions).length} available`,
    );
    console.log(
      `React Components: ${totalReactComponents}/${Object.keys(componentCheck.reactComponents).length} present`,
    );
    console.log(
      `AWS Configuration: ${awsCheck.awsmobileExists ? "Working" : "Failed"}`,
    );
    ...
    const overallScore =
      (totalUIElements +
        ...
        (awsCheck.awsmobileExists ? 1 : 0)) /
      (Object.keys(componentCheck.uiElements).length +
        Object.keys(componentCheck.criticalFunctions).length +
        Object.keys(componentCheck.reactComponents).length +
        1);
    ...
    if (grade >= "B") {
      ...
      console.log(
        "⚠️ WARNING: Some issues detected with components in production.",
      );
    }
  } catch (error) {
