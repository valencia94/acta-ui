...
async function testJWTTokens() {
    ...
    const tokenDetails = await page.evaluate(async () => {
        try {
            ...
            return {
                ...
                amplifyToken: amplifyToken ? amplifyToken.substring(0, 50) + '...' : null,
    ...
    const apiTestResult = await page.evaluate(async () => {
        try {
            ...
            const response = await fetch('https://q2b9avfwv5.execute-api.us-east-2.amazonaws.com/prod/download-acta/test-project-001?format=pdf', {
                method: 'GET',
                headers: {
