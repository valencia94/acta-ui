...
console.log(
  "Skip Auth Mode:",
  window.location.search.includes("skip") || "Check .env file",
);
...
function testUserAuth() {
  ...
  const userMenuEl = document.querySelector(
  ...
  if (adminButtons.length > 0) {
    ...
    Array.from(adminButtons).forEach((btn, i) => {
      console.log(
        `  Admin element ${i + 1}:`,
        btn.textContent?.trim() || btn.outerHTML.substring(0, 50),
      );
    });
  } else {
...
function testRoleAccess() {
  ...
  const adminLink = document.querySelector(
    `[href="${adminPath}"], [onclick*="admin"]`,
  );
  if (adminLink) {
    console.log(
      "✅ Admin navigation link found:",
      adminLink.textContent?.trim(),
    );
  } else {
  ...
  if (currentPath === adminPath) {
    ...
    console.log(
      "ℹ️ On regular dashboard - try clicking admin link in header menu",
    );
  }
}
...
function testButtons() {
  ...
  const actionButtons = document.querySelectorAll(
    'button:not([class*="menu"]):not([class*="header"])',
  );
...
async function runAllTests() {
  ...
  console.log("\n🎯 NEXT STEPS:");
  console.log(
    "1. Try accessing /admin directly: " + window.location.origin + "/admin",
  );
