...
async function testFrontendFetchConfiguration() {
  ...
  try {
    ...
    console.log(
      `✅ Fetch successful! Status: ${response.status} ${response.statusText}`,
    );
    ...
    if (response.status === 401) {
      console.log(
        "✅ Expected 401 - Authentication token is invalid (expected)",
      );
    } else {
    ...
    if (
      ...
      try {
        ...
        console.log(
          `✅ Basic connectivity OK: ${basicResponse.status} ${basicResponse.statusText}`,
        );
      } catch (basicError) {
  ...
  try {
    ...
    console.log(
      `✅ Fetch without credentials successful! Status: ${response.status} ${response.statusText}`,
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ Fetch with Origin header successful! Status: ${response.status} ${response.statusText}`,
    );
  } catch (error) {
