...
async function testBasicConnectivity() {
  ...
  try {
    ...
    addTestResult(
      ...
      healthResponse.statusCode !== 200 ? `Status: ${healthResponse.statusCode}` : ''
    );
    ...
    const isAPIGateway = healthResponse.headers['x-amzn-requestid'] || 
...
async function testProjectEndpoints() {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      const success = response.statusCode === 200 || response.statusCode === 401; // 401 is expected without auth
      addTestResult(
        `Endpoint ${endpoint}`, 
        success,
        !success ? `Status: ${response.statusCode}` : `Status: ${response.statusCode}`
      );
...
async function testAuthenticationFlow() {
  ...
  try {
    ...
    addTestResult(
      ...
      success,
      !success ? `Status: ${response.statusCode}` : 'Service responding (auth required)'
    );
...
async function testAPIGatewayCORS() {
  ...
  try {
    ...
    for (const header of requiredHeaders) {
      const exists = corsHeaders[header] || corsHeaders[header.replace(/-/g, '_')];
...
async function testSpecificAPIIssues() {
  ...
  try {
    ...
    console.log('Testing the exact failing endpoint from the production test...');
    ...
    addTestResult(
      ...
      reachable,
      !reachable ? `Status: ${response.statusCode}` : `Status: ${response.statusCode} (reachable)`
    );
...
async function checkNetworkPolicies() {
  ...
  try {
    ...
    const successfulRequests = rapidResults.filter(r => 
...
function generateReport() {
  ...
  const apiGatewayAccessible = testResults.details.some(t => 
    t.testName.includes('Health endpoint') && t.passed
  );
  ...
  const corsIssues = testResults.details.some(t => 
    t.testName.includes('CORS') && !t.passed
  );
  
  if (corsIssues) {
    console.log('⚠️  CORS configuration needs attention');
    console.log('   - Update API Gateway CORS settings');
    console.log('   - Add https://d7t9x3j66yd8k.cloudfront.net to allowed origins');
  }
  
  const authIssues = testResults.details.some(t => 
    t.testName.includes('Authentication') && !t.passed
  );
