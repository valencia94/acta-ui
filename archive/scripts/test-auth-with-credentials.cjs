...
async function testAuthenticationFlow() {
  ...
  if (!TEST_EMAIL || !TEST_PASSWORD) {
    console.error(
      '❌ Missing credentials: ACTA_UI_USER and ACTA_UI_PW environment variables required'
    );
    return false;
  }
  ...
  try {
    ...
    if (!initialAuthState.hasLoginForm) {
      console.log(
        '❌ No login form found. Checking for other auth elements...'
      );
    ...
    if (!submitButton) {
      ...
      const loginButton = await page.$(
        'button:contains("Sign In"), button:contains("Login"), button:contains("Sign in")'
      );
      if (loginButton) {
    ...
    const isAuthenticated =
      !postLoginState.hasLoginForm ||
      postLoginState.debugText?.includes('authenticated') ||
      !postLoginState.hasEmailInput;

    console.log(
      `🔐 Authentication Status: ${isAuthenticated ? '✅ SUCCESS' : '❌ FAILED'}`
    );
...
async function main() {
  ...
  if (!diagnostics.credentialsAvailable) {
    console.error(
      '❌ Cannot proceed without credentials. Please set ACTA_UI_USER and ACTA_UI_PW environment variables.'
    );
    process.exit(1);
  }
  ...
  console.log(
    `CORS Support: ${diagnostics.corsSupported ? '✅ PASS' : '❌ FAIL'}`
  );
  console.log(
    `Credentials Available: ${diagnostics.credentialsAvailable ? '✅ PASS' : '❌ FAIL'}`
  );
