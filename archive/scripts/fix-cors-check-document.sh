...
# Fix CORS for /check-document/{projectId} endpoint
...
echo "🎯 Fixing CORS for /check-document/{projectId} endpoint..."
echo "   Origin: $PROD_ORIGIN"
echo ""

# Get resource ID for check-document/{projectId}
echo "🔍 Finding resource ID for /check-document/{projectId}..."
RESOURCE_ID=$(aws apigateway get-resources --rest-api-id "$API_ID" --region "$REGION" \
    --query "items[?contains(path, 'check-document') && contains(path, '{projectId}')].id | [0]" --output text)

if [[ "$RESOURCE_ID" == "None" || -z "$RESOURCE_ID" ]]; then
    echo "❌ Could not find resource for /check-document/{projectId}"
    exit 1
fi
...
aws apigateway create-deployment \
    --rest-api-id "$API_ID" \
    --stage-name "prod" \
    --description "Fix CORS for /check-document/{projectId} endpoint" \
    --region "$REGION"

echo ""
echo "✅ CORS Fixed for /check-document/{projectId}!"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🎯 Fixed: /check-document/{projectId}"
