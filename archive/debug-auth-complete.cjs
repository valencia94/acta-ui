...
async function debugAuthenticationFlow() {
  ...
  try {
    ...
    const wellKnownTest = await new Promise((resolve, reject) => {
      const req = https.get(
        `https://cognito-idp.us-east-2.amazonaws.com/us-east-2_FyHLtOhiY/.well-known/jwks.json`,
        (res) => {
          let data = '';
    ...
    console.log(
      `✅ Cognito JWKS: ${wellKnownTest.status} ${wellKnownTest.valid ? '(Valid)' : '(Invalid)'}`
    );
    ...
    console.log(
      `📊 No Auth Header: ${emptyAuthTest.status} - Expected 401/403`
    );

    // Test with invalid auth header
    const invalidAuthTest = await testAPICall(
      '/projects',
      'Bearer invalid-token'
    );
    console.log(
      `📊 Invalid Token: ${invalidAuthTest.status} - Expected 401/403`
    );
