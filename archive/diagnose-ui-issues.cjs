...
async function diagnoseUIIssues() {
  ...
  try {
    ...
    if (awsExportsTest.status === 200) {
      console.log(
        `🔄 Has Logout Redirect: ${awsExportsTest.hasLogoutRedirect}`
      );

      // Extract redirect URLs
      const redirectMatches = awsExportsTest.content.match(
        /redirectSign(In|Out):\s*['"]([^'"]+)['"]/g
      );
      if (redirectMatches) {
    ...
    const testEndpoints = [
      ...
      {
        path: '/pm-manager/projects/1000000049842296',
    ...
    for (const endpoint of testEndpoints) {
      try {
              ...
              resolve({
                ...
                headers: Object.fromEntries(
                  res.rawHeaders
                    .map((h, i) =>
                      i % 2 === 0
                        ? [h.toLowerCase(), res.rawHeaders[i + 1]]
                        : null
                    )
                    .filter(Boolean)
                ),
        ...
        if (response.status === 403 || response.status === 401) {
          console.log(
            `   ⚠️ Auth required - ${response.data.substring(0, 100)}`
          );
    ...
    const envTest = await new Promise((resolve, reject) => {
      const req = https.get(
        `${FRONTEND_URL}/assets/index-B5NTLSft.js`,
        (res) => {
          let data = '';
          res.on('data', (chunk) => (data += chunk));
          res.on('end', () =>
      ...
      req.setTimeout(15000, () =>
        reject(new Error('Environment test timeout'))
      );
    });
    ...
    for (const endpoint of docEndpoints) {
      try {
        ...
        console.log(
          `📋 ${endpoint}: ${response.status} (${response.contentType})`
        );
        ...
        (res) => {
          resolve({
            ...
            headers: Object.fromEntries(
              res.rawHeaders
                .map((h, i) =>
                  i % 2 === 0 ? [h.toLowerCase(), res.rawHeaders[i + 1]] : null
                )
                .filter(Boolean)
            ),
          });
    ...
    console.log(
      `   Access-Control-Allow-Origin: ${corsTest.headers['access-control-allow-origin'] || 'Not set'}`
    );
    console.log(
      `   Access-Control-Allow-Methods: ${corsTest.headers['access-control-allow-methods'] || 'Not set'}`
    );
    console.log(
      `   Access-Control-Allow-Headers: ${corsTest.headers['access-control-allow-headers'] || 'Not set'}`
    );
    ...
    if (
      corsTest.headers['access-control-allow-origin'] !== FRONTEND_URL &&
    ...
    return {
      ...
      fixes: ['Check network connectivity'],
      logoutRedirectOK: false,
      apiAuthOK: false,
      envConfigOK: false,
    };
  }
...
if (require.main === module) {
  diagnoseUIIssues()
    .then((result) => {
      const allOK =
        result.logoutRedirectOK && result.apiAuthOK && result.envConfigOK;
      console.log(
