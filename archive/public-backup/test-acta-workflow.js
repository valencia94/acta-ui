...
async function testDocumentGeneration() {
  ...
  try {
    ...
    const generateResponse = await fetch(
      `${API_BASE_URL}/extract-project-place/${TEST_PROJECT_ID}`,
      {
        method: 'POST',
        headers: {
    ...
    console.log(
      `📊 Generation response: ${generateResponse.status} ${generateResponse.statusText}`
    );
...
async function testDocumentDownload(format = 'docx') {
  ...
  try {
    ...
    console.log(
      `🔍 Checking document availability for project: ${TEST_PROJECT_ID}`
    );

    const checkResponse = await fetch(
      `${API_BASE_URL}/check-document/${TEST_PROJECT_ID}?format=${format}`,
      {
        method: 'HEAD',
      }
    );
    ...
    console.log(
      `📊 Document check response: ${checkResponse.status} ${checkResponse.statusText}`
    );
    ...
    const downloadResponse = await fetch(
      `${API_BASE_URL}/download-acta/${TEST_PROJECT_ID}?format=${format}`,
      {
        method: 'GET',
        redirect: 'manual',
      }
    ...
    console.log(
      `📊 Download response: ${downloadResponse.status} ${downloadResponse.statusText}`
    );
    console.log(
      '📋 Response headers:',
    ...
    console.log(
      `📊 URL test response: ${urlTest.status} ${urlTest.statusText}`
    );
    ...
    if (urlTest.ok) {
      ...
      console.log(
        `📏 Content-Length: ${urlTest.headers.get('Content-Length')}`
      );
      return true;
    } else {
...
async function testS3DirectAccess() {
  ...
  try {
    ...
    console.log(
      `📊 S3 response: ${s3Response.status} ${s3Response.statusText}`
    );
...
window.testActaWorkflow = {
  ...
  quickTest: async (apiUrl, projectId) => {
    ...
    console.log(
      `🧪 Quick test with API: ${API_BASE_URL}, Project: ${TEST_PROJECT_ID}`
    );
