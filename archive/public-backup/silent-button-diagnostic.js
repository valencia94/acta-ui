...
function diagnoseSilentButtons() {
  ...
  console.log(
    'Expected API URL: https://q2b9avfwv5.execute-api.us-east-2.amazonaws.com/prod'
  );
  ...
  buttons.forEach((btn, index) => {
    ...
    const hasEventListeners = getEventListeners
      ? getEventListeners(btn).click
      : 'Unknown';

    console.log(
      `Button ${index + 1}: "${text}" | Disabled: ${disabled} | onClick: ${hasClick}`
    );
  ...
  inputs.forEach((input, index) => {
    console.log(
      `Input ${index + 1}: Value="${input.value}" | Placeholder="${input.placeholder}"`
    );
  });
...
function fillProjectIdAndTest() {
  ...
  const projectInput = document.querySelector(
    'input[placeholder*="project"], input[id*="project"], #projectId'
  );
