...
async function fixApiGatewayConfiguration() {
  ...
  if (authorizers && authorizers.items) {
    const cognitoAuth = authorizers.items.find(
      (auth) =>
    ...
    if (cognitoAuth) {
      ...
      console.log(
        `✅ Found Cognito Authorizer: ${cognitoAuth.name} (ID: ${cognitoAuthorizerId})`,
      );
    }
  }
  ...
  if (!cognitoAuthorizerId) {
    ...
    if (createAuthResult) {
      ...
      console.log(
        `✅ Created Cognito Authorizer with ID: ${cognitoAuthorizerId}`,
      );
    }
  }
  ...
  for (const endpoint of protectedEndpoints) {
    ...
    const resource = resources.items.find(
      (r) => r.pathPart && endpoint.path.includes(r.pathPart),
    );
    ...
    for (const method of endpoint.methods) {
      try {
        ...
        console.log(
          `   ⚠️ Method ${method} may not exist for ${endpoint.path}`,
        );
      }
    }
  }
  ...
  if (deployResult) {
    console.log(
      `✅ API deployed successfully! Deployment ID: ${deployResult.id}`,
    );
  }
