...
async function testLiveAuthentication() {
  ...
  try {
    ...
    console.log(
      `⚛️ React Root: ${frontendTest.hasReactRoot ? 'Found' : 'Missing'}`
    );
    console.log(
      `🔧 Amplify Bundle: ${frontendTest.hasAmplifyBundle ? 'Found' : 'Missing'}`
    );
    ...
    if (frontendTest.size < 1000) {
      ...
      const jsTest = await new Promise((resolve, reject) => {
        const req = https.get(
          `${FRONTEND_URL}/assets/index-DEyieCy3.js`,
          (res) => {
            let data = '';
            res.on('data', (chunk) => (data += chunk));
            res.on('end', () =>
              resolve({
                ...
                size: data.length,
                hasAmplify:
                  data.includes('aws-amplify') || data.includes('cognito'),
              })
            );
          }
        );
    ...
    for (const endpoint of protectedEndpoints) {
      try {
        ...
        console.log(
          `${isAuthRequired ? '✅' : '❌'} ${endpoint}: ${response.status} ${isAuthRequired ? '(Auth required)' : '(No auth - Issue!)'}`
        );
      } catch (error) {
    ...
    try {
      const docCheck = await new Promise((resolve, reject) => {
        const req = https.get(
          `${API_BASE}/check-document/${testProjectId}`,
          (res) => {
            let data = '';
            res.on('data', (chunk) => (data += chunk));
            res.on('end', () =>
        ...
        req.setTimeout(10000, () =>
          reject(new Error('Document check timeout'))
        );
      });

      console.log(
        `📋 Document Check: ${docCheck.status} - ${docCheck.data.substring(0, 200)}`
      );
    } catch (error) {
    ...
    console.log(
      `Frontend Deployment: ${frontendOK ? '✅ SUCCESS' : '❌ FAILED'}`
    );
    console.log(`API Backend: ${apiOK ? '✅ SUCCESS' : '❌ FAILED'}`);
    console.log(
      `Authentication Required: ${authRequired ? '✅ SUCCESS' : '❌ FAILED'}`
    );
    console.log(
      `Bundle Size: ${frontendTest.size > 1000 ? '✅ PROPER' : '⚠️ CACHED'}`
    );
