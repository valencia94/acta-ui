...
async function testCognitoAuth() {
  ...
  const result = await page.evaluate(async () => {
    ...
    try {
      ...
      const testEndpoints = [
        {
          name: "getSummary",
          func: window.getSummary,
          params: ["1000000064013473"],
        },
        {
          name: "getTimeline",
          func: window.getTimeline,
          params: ["1000000064013473"],
        },
        {
          name: "fetchWrapper",
          func: window.fetchWrapper,
          params: ["/health"],
        },
      ];
      ...
      results.apiAuthenticationWorking = results.apiCallResults.some(
        (r) => r.status === "SUCCESS",
      );
    } catch (error) {
  ...
  console.log(
    ...
    result.apiAuthenticationWorking ? "✅" : "❌",
  );

  console.log("\n=== API CALL RESULTS ===");
  result.apiCallResults.forEach((api) => {
    console.log(
      `${api.endpoint}: ${api.status === "SUCCESS" ? "✅" : "❌"} ${api.status}`,
    );
    if (api.error) {
