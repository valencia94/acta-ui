...
const tests = {
  ...
  testProjectsFetch: async function () {
    ...
    try {
      ...
      for (const projectId of testProjectIds) {
        ...
        console.log(
          `   Planlet: ${data.planlet_name || data.planlet || "N/A"}`,
        );
      }
      return true;
    } catch (error) {
  ...
  testGenerateActa: async function (projectId = testProjectIds[0]) {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/extract-project-place/${projectId}`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
  ...
  testDownloadPDF: async function (projectId = testProjectIds[0]) {
    ...
    try {
      ...
      const downloadResponse = await fetch(
        `${API_BASE}/download-acta/${projectId}?format=pdf`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
  ...
  testDownloadDOCX: async function (projectId = testProjectIds[0]) {
    ...
    try {
      ...
      // Then get download URL
      const downloadResponse = await fetch(
        `${API_BASE}/download-acta/${projectId}?format=docx`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
