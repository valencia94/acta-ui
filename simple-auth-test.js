...
(async () => {
  ...
  if (hasEmailInput && hasPasswordInput && hasSignInButton) {
    ...
    const errorElement = await page.$(
      '.error, [class*="error"], [data-testid*="error"], [role="alert"]',
    );
    if (errorElement) {
    ...
    const authInfo = await page.evaluate(() => {
      return {
        ...
        cognitoTokens: Object.keys(localStorage).filter((key) =>
          key.includes("CognitoIdentityServiceProvider"),
        ),
        hasAmplifyTokens: Object.keys(localStorage).some((key) =>
    ...
    console.log(
      "  - JWT token:",
      authInfo.jwtToken ? "✅ Present" : "❌ Missing",
    );
