...
const getSizeKB = file => (fs.statSync(path.join(assetsDir, file)).size / 1024).toFixed(1);
...
if (indexSize > 1024) {
  console.warn(`⚠️ index.js is large (${indexSize.toFixed(1)} KB). Confirm chunking is effective.`);
} else {
