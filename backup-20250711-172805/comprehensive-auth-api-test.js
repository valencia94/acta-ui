...
try {
  ...
  console.log('📋 Cognito Pool ID:', awsConfig.aws_user_pools_id || 'Not found');
  console.log('📋 Client ID:', awsConfig.aws_user_pools_web_client_id || 'Not found');
...
async function testProtectedEndpoint() {
  try {
    ...
    if (response.status === 401) {
      console.log('✅ Auth protection working (401 expected without valid token)');
...
function testButtonsPresence() {
  const buttons = {
    'Generate All Acta': document.querySelector('[data-testid="generate-all-acta"], button[title*="Generate"], button:contains("Generate All")'),
    'Download Word': document.querySelector('[data-testid="download-word"], button[title*="Word"], button:contains("Word")'),
    'Download PDF': document.querySelector('[data-testid="download-pdf"], button[title*="PDF"], button:contains("PDF")'),
    'Preview PDF': document.querySelector('[data-testid="preview-pdf"], button[title*="Preview"], button:contains("Preview")'),
    'Send Approval': document.querySelector('[data-testid="send-approval"], button[title*="Approval"], button:contains("Approval")')
  };
...
function testReactComponents() {
  const components = {
    ...
    'Dashboard': document.querySelector('[data-component="dashboard"], .dashboard, [class*="dashboard"]'),
    'Header': document.querySelector('header, [data-component="header"], .header'),
    'Project Input': document.querySelector('input[placeholder*="Project"], input[name*="project"]'),
  ...
  Object.entries(components).forEach(([name, element]) => {
    console.log(`${element ? '✅' : '❌'} ${name}: ${element ? 'Mounted' : 'Not found'}`);
  });
...
console.log(`${actualTitle === expectedTitle ? '✅' : '❌'} Title ${actualTitle === expectedTitle ? 'correct' : 'incorrect'}`);
