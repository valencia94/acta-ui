...
console.log(
  '🔧 Fixed: Authentication headers, endpoint paths, and error handling'
);
...
async function runTests() {
  ...
  const tests = [
    ...
    [
      `${API_BASE_URL}/generate-acta/test`,
      'Generate ACTA (test)',
      { expectAuth: true, method: 'POST', body: {} },
    ],

    // Backend endpoints (may have Lambda issues)
    [
      `${API_BASE_URL}/project-summary/test`,
    ...
    [
      `${API_BASE_URL}/download-acta/test?format=pdf`,
      'Download ACTA PDF (test)',
      { expectAuth: true },
    ],
    [
      `${API_BASE_URL}/download-acta/test?format=docx`,
      'Download ACTA DOCX (test)',
      { expectAuth: true },
    ],

    // Alternative endpoint patterns (in case API uses different paths)
    [
      `${API_BASE_URL}/extract-project-place/test`,
      'Extract Project Data (test)',
      { expectAuth: true, method: 'POST', body: {} },
    ],
    [
      `${API_BASE_URL}/check-document/test?format=pdf`,
      'Check Document Status (test)',
      { expectAuth: true, method: 'HEAD' },
    ],
  ];
  ...
  if (working > 0 && authProtected > 0) {
    ...
    if (failed > 0) {
      console.log(
        '⚠️ Some backend Lambda functions need attention (502 errors)'
      );
    }
...
async function testCORSAndConnectivity() {
  ...
  console.log(
    '2. 🔧 For 502 errors: Check CloudWatch logs for Lambda functions'
  );
