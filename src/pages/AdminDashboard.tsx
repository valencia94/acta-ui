  ...
  useEffect(() => {
    ...
    const runDiagnostic = async () => {
      try {
        ...
        if (!isBackendWorking) {
          toast.error('Backend API connectivity issues detected. Check system status.', {
            duration: 8000,
          });
        }
      } catch (e) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <div className="flex items-center bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-2">
            ...
            <span className="text-sm font-medium text-indigo-800">Admin Access: {user?.email}</span>
          </div>
        </motion.div>
