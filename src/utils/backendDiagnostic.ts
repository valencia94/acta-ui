...
export async function quickBackendDiagnostic(): Promise<boolean> {
  ...
  if (apiBaseUrl.includes('localhost')) {
    ...
    console.log('🔧 If localhost backend is not running, some features will be disabled');
  }
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      console.error('❌ Backend API timeout - connection is too slow or unavailable');
    } else {
      console.error('❌ Cannot reach backend API:', error.message);
    }
    console.log('💡 You can still use manual project entry and generate individual Actas');
    return false;
  }
}
...
if (typeof window !== 'undefined') {
  (window as unknown as Record<string, unknown>).quickBackendDiagnostic = quickBackendDiagnostic;
}
