...
async function testAPIConnectivity() {
  ...
  try {
    ...
    if (session.tokens?.idToken) {
      console.log('ID Token (first 50 chars):', session.tokens.idToken.toString().substring(0, 50) + '...');
    }
  } catch (error) {
...
console.log('\n🎯 Diagnostic Summary:');
console.log('1. If health check returns 403: API requires authentication for all endpoints');
...
console.log('4. If auth session exists but still 403: Token format or validation issue');
