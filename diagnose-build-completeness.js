...
const distFiles = findFiles('dist', ['.js', '.css', '.html', '.png', '.svg', '.ico']);
...
Object.keys(distByType).sort().forEach(ext => {
...
const orphanedFiles = [];
const tsxFiles = srcFiles.filter(file => file.endsWith('.tsx') || file.endsWith('.ts'));
...
try {
  ...
  const assetFiles = distFiles.filter(f => !f.endsWith('.js') && !f.endsWith('.css') && !f.endsWith('.html'));
  ...
  if (mainBundle) {
    const bundleSize = execSync(`du -sh "${mainBundle}"`, { encoding: 'utf-8' }).trim();
...
if (orphanedFiles.length > 0) {
  recommendations.push(`Remove ${orphanedFiles.length} orphaned files to clean up the build`);
}

const missingCritical = criticalComponents.filter(c => !fs.existsSync(c));
if (missingCritical.length > 0) {
  recommendations.push(`${missingCritical.length} critical components are missing`);
}

const notImportedCritical = criticalComponents.filter(c => 
  fs.existsSync(c) && !isFileImported(c, srcFiles)
);
if (notImportedCritical.length > 0) {
  recommendations.push(`${notImportedCritical.length} critical components are not imported`);
}
...
console.log('\n🎯 Build Completeness: ' + (recommendations.length === 0 ? 'GOOD' : 'NEEDS ATTENTION'));
