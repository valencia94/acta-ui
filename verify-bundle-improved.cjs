...
function checkBundleContents() {
  ...
  console.log(`📦 Main bundle: ${mainBundle} (${(largestSize / 1024).toFixed(2)} KB)`);
  ...
  function findInBundle(name, category) {
    const patterns = [
      ...
      name.replace(/([A-Z])/g, '-$1').toLowerCase().substring(1), // kebab-case
      name.replace(/([A-Z])/g, '_$1').toLowerCase().substring(1), // snake_case
      `"${name}"`,                            // quoted
      `'${name}'`,                            // single quoted
  ...
  const patterns = [
    { name: 'React', searches: ['react', 'createElement', 'useState', 'useEffect'] },
    { name: 'Framer Motion', searches: ['framer', 'motion.', 'animate'] },
    { name: 'Lucide Icons', searches: ['lucide', 'FileText', 'Download', 'Plus'] },
    { name: 'AWS Amplify', searches: ['amplify', 'aws-amplify', 'fetchAuthSession'] },
    { name: 'Chakra UI', searches: ['chakra', 'ChakraProvider'] },
    { name: 'React Router', searches: ['router', 'BrowserRouter', 'Routes', 'Navigate'] },
    { name: 'Tailwind CSS', searches: ['bg-gradient', 'text-', 'px-', 'rounded-', 'shadow'] },
    { name: 'React Hot Toast', searches: ['toast', 'Toaster', 'react-hot-toast'] }
  ];
  
  patterns.forEach(pattern => {
    const found = pattern.searches.some(search => bundleContent.includes(search));
    console.log(`  ${found ? '✅' : '❌'} ${pattern.name}: ${found ? 'Present' : 'Missing'}`);
  });
  ...
  const functionalityPatterns = [
    ...
    { name: 'Document Generation', searches: ['document', 'pdf', 'docx', 'acta'] },
  ...
  console.log(`  🔤 Content length: ${bundleContent.length.toLocaleString()} chars`);
  
  // Look for minification indicators
  const isMinified = bundleContent.includes('var ') || 
  ...
  console.log('=' .repeat(60));
  console.log(`Components found: ${foundComponents}/${components.length} (${((foundComponents/components.length)*100).toFixed(1)}%)`);
  console.log(`API functions found: ${foundApiFunctions}/${apiFunctions.length} (${((foundApiFunctions/apiFunctions.length)*100).toFixed(1)}%)`);
  console.log(`Utilities found: ${foundUtilities}/${utilities.length} (${((foundUtilities/utilities.length)*100).toFixed(1)}%)`);
  
  const totalExpected = components.length + apiFunctions.length + utilities.length;
  ...
  if (overallPercentage >= 90) {
    console.log('🎉 EXCELLENT: Bundle appears to contain all critical components!');
  } else if (overallPercentage >= 70) {
    console.log('✅ GOOD: Most critical components are present in the bundle.');
  } else if (overallPercentage >= 50) {
    console.log('⚠️ WARNING: Some critical components may be missing from the bundle.');
  } else {
    console.log('❌ CRITICAL: Many expected components are missing from the bundle.');
  }
