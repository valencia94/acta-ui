...
async function runFullTestSuite() {
  ...
  results.summary = await testApiEndpoint(
    `/project-summary/${CONFIG.testProjectId}`,
  );
  console.log("");
  ...
  results.generate = await testApiEndpoint(
    `/extract-project-place/${CONFIG.testProjectId}`,
    "POST",
    generatePayload,
  );
  console.log("");
  ...
  results.download = await testApiEndpoint(
    `/download-acta/${CONFIG.testProjectId}?format=pdf`,
  );
  console.log("");

  // 6. Test Document Check
  console.log("6️⃣ Testing Document Check...");
  results.check = await testApiEndpoint(
    `/check-document/${CONFIG.testProjectId}?format=pdf`,
  );
  console.log("");
  ...
  results.approval = await testApiEndpoint(
    "/send-approval-email",
    "POST",
    emailPayload,
  );
  ...
  Object.entries(results).forEach(([test, result]) => {
    ...
    console.log(
      `${status} ${test.toUpperCase()}: ${httpStatus} (${duration}ms)`,
    );
  });
  ...
  console.log(
    `🔐 Authentication: ${authPassing}/${authTests.length} endpoints working`,
  );
  ...
  if (results.generate?.success) {
    ...
    console.log(
      "📝 Generate ACTA: Project not found - try different Project ID",
    );
  } else {
...
console.log(
  "  testAWSAPIs.testApiEndpoint(endpoint, method, payload) - Test specific endpoint",
);
console.log("");
