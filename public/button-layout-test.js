...
function testButtonLayout() {
  ...
  if (!actaSection) {
    console.log('❌ Generate Acta section not found. Make sure you\'re in Manual Entry mode.');
    return;
  }
  ...
  if (buttonContainer) {
    ...
    if (buttons.length > 0) {
      ...
      const searchInput = document.querySelector('input[placeholder*="project ID"]');
      if (searchInput) {
