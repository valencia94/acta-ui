...
async function runAllTests() {
    ...
    try {
        ...
        const userEmail = document.querySelector('[data-testid="user-email"], .user-email') || 
                         Array.from(document.querySelectorAll('*')).find(el => 
                             el.textContent && el.textContent.includes('@') && el.textContent.includes('.com')
                         );
        ...
        const projectInput = document.querySelector('input[placeholder*="Project"], input[placeholder*="project"], input[type="text"]') ||
            ...
            if (button) {
                ...
                try {
                    ...
                    results.buttons[buttonType] = {
                        ...
                        apiCalls: fetchCalls.filter(call => 
        ...
        const apiCalls = results.apiCalls.filter(call => 
            call.url.includes('execute-api') || 
            call.url.includes('amazonaws.com')
        );
        ...
        const modalCheck = document.querySelector('[role="dialog"], .modal, .pdf-preview') !== null;
        console.log(`PDF Preview Modal: ${modalCheck ? '✅ Found' : '❌ Not found'}`);
        
        const toastCheck = document.querySelector('.toast, .notification, [data-sonner-toaster]') !== null;
        console.log(`Toast Notifications: ${toastCheck ? '✅ Found' : '❌ Not found'}`);
    ...
    const successfulButtons = Object.values(results.buttons).filter(b => b.status === 'clicked').length;
    const apiCallsCount = results.apiCalls.filter(call => 
        call.url.includes('execute-api') || call.url.includes('amazonaws.com')
    ).length;
    
    console.log(`Authentication: ${results.authentication === 'logged_in' ? '✅' : '❌'} ${results.authentication}`);
    console.log(`Project Input: ${results.projectInput === 'found_and_set' ? '✅' : '❌'} ${results.projectInput}`);
    ...
    Object.entries(results.buttons).forEach(([buttonType, result]) => {
        const status = result.status === 'clicked' ? '✅' : 
    ...
    console.log('\n💡 Next Steps:');
    console.log('1. Check the Network tab in DevTools for detailed API responses');
