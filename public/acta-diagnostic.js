...
const isProd = currentUrl.includes('acta-ui-frontend-prod') || currentUrl.includes('cloudfront');

console.log(`🌍 Environment: ${isLocal ? 'Local Development' : isProd ? 'Production' : 'Unknown'}`);
...
async function diagnoseDocumentGeneration(projectId = TEST_PROJECT_ID) {
    ...
    console.log('Expected flow: UI → API Gateway → Lambda → External Data → DOCX Creation → S3 Storage');
    
    try {
        console.log('📤 Step 1: Triggering document generation...');
        
        const startTime = Date.now();
        const response = await fetch(`${API_BASE_URL}/extract-project-place/${projectId}`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                console.log('💡 Possible causes: API endpoint not found, incorrect URL, or Lambda not deployed');
            } else if (response.status === 500) {
                console.log('💡 Possible causes: Lambda function error, external data source unavailable, or S3 permissions issue');
            } else if (response.status === 403) {
                console.log('💡 Possible causes: Authentication issue or insufficient permissions');
            }
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            console.log('💡 Possible causes: CORS issue, API Gateway down, or incorrect API URL');
        }
...
async function diagnoseDocumentDownload(projectId = TEST_PROJECT_ID, format = 'docx') {
    console.log(`\n📥 Diagnosing Document Download for project: ${projectId}, format: ${format}`);
    console.log('Expected flow: UI → API Gateway → S3 Signed URL → Document Download');
    
    try {
        console.log(`🔍 Step 1: Checking if document exists in S3...`);
        
        // Check document availability
        const checkResponse = await fetch(`${API_BASE_URL}/check-document/${projectId}?format=${format}`, {
            method: 'HEAD'
        });
        
        console.log(`📊 Document check: ${checkResponse.status} ${checkResponse.statusText}`);
        
        if (checkResponse.status === 404) {
            console.log('📄 Document not found in S3 - need to generate first');
            return { success: false, error: 'Document not found', needsGeneration: true };
        }
        ...
        const downloadResponse = await fetch(`${API_BASE_URL}/download-acta/${projectId}?format=${format}`, {
            method: 'GET',
            redirect: 'manual'
        });
        
        console.log(`📊 Download API response: ${downloadResponse.status} ${downloadResponse.statusText}`);
        console.log(`📋 Response headers:`, Object.fromEntries(downloadResponse.headers.entries()));
        
        if (downloadResponse.status !== 302) {
            const errorText = await downloadResponse.text();
            console.error(`❌ Expected 302 redirect, got ${downloadResponse.status}:`);
            console.error(errorText);
            
            if (downloadResponse.status === 404) {
                console.log('💡 Document not found in S3 - may need to generate first');
            } else if (downloadResponse.status === 500) {
                console.log('💡 Possible causes: S3 access error, Lambda error, or missing document');
            }
            
            return { success: false, error: errorText, status: downloadResponse.status };
        }
        ...
        console.log(`📊 S3 URL test: ${s3Response.status} ${s3Response.statusText}`);
        
        if (s3Response.ok) {
            console.log('✅ S3 signed URL is accessible');
            console.log(`📂 Content-Type: ${s3Response.headers.get('Content-Type')}`);
            console.log(`📏 Content-Length: ${s3Response.headers.get('Content-Length')} bytes`);
            console.log(`📅 Last-Modified: ${s3Response.headers.get('Last-Modified')}`);
            ...
            if (contentType && contentType.includes(expectedType.split('/')[1])) {
                ...
                console.log(`⚠️ Content type mismatch. Expected: ${expectedType}, Got: ${contentType}`);
            }
            
            return { success: true, s3SignedUrl, contentType, size: s3Response.headers.get('Content-Length') };
        } else {
            console.error(`❌ S3 signed URL not accessible: ${s3Response.statusText}`);
            
            if (s3Response.status === 403) {
                console.log('💡 Possible causes: Signed URL expired, S3 permissions issue, or incorrect bucket');
            } else if (s3Response.status === 404) {
                console.log('💡 Document not found in S3 - may have been deleted or never created');
            }
            
            return { success: false, error: 'S3 URL not accessible', status: s3Response.status };
        }
...
async function diagnoseCompleteWorkflow(projectId = TEST_PROJECT_ID) {
    ...
    if (results.generation.success) {
        ...
        } else if (results.docxDownload?.needsGeneration || results.pdfDownload?.needsGeneration) {
    ...
    console.log(`📄 DOCX Download: ${results.docxDownload?.success ? '✅ Success' : results.docxDownload?.needsGeneration ? '⏳ Needs Generation' : '❌ Failed'}`);
    console.log(`📄 PDF Download: ${results.pdfDownload?.success ? '✅ Success' : results.pdfDownload?.needsGeneration ? '⏳ Needs Generation' : '❌ Failed'}`);
...
window.actaDiagnostic = {
    ...
    // Test specific parts
    testGeneration: (projectId) => diagnoseDocumentGeneration(projectId || TEST_PROJECT_ID),
    testDocx: (projectId) => diagnoseDocumentDownload(projectId || TEST_PROJECT_ID, 'docx'),
    testPdf: (projectId) => diagnoseDocumentDownload(projectId || TEST_PROJECT_ID, 'pdf')
};
