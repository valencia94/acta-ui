...
async function testProductionSite() {
  ...
  try {
    ...
    const devToolsAnalysis = await page.evaluate(async () => {
      ...
      try {
        ...
        analysis.criticalErrors.push(
          "UI Structure Analysis Failed: " + error.message,
        );
      }
      ...
      try {
        ...
        const missingResources = criticalResources.filter((resource) => {
          return !Array.from(document.querySelectorAll("script")).some(
            (script) => script.src && script.src.includes(resource),
          );
        });

        if (missingResources.length > 0) {
          analysis.criticalErrors.push(
            "Missing critical resources: " + missingResources.join(", "),
          );
        }
        ...
        const failedRequests = performance
          .getEntries()
          .filter(
      ...
      try {
        ...
        analysis.performance = {
          ...
          resourcesLoaded: performance
            .getEntries()
            .filter((entry) => entry.entryType === "resource").length,
      ...
      try {
        ...
        analysis.warnings.push(
          "Accessibility Analysis Failed: " + error.message,
        );
      }
      ...
      try {
        analysis.security = {
          ...
          mixedContent: Array.from(
            document.querySelectorAll("script, link, img"),
          ).some((el) => {
            const src = el.src || el.href;
            return src && src.startsWith("http://");
          }),
          cspPresent: !!document.querySelector(
    ...
    console.log(
      `  📄 Total DOM Elements: ${devToolsAnalysis.uiStructure.totalElements}`,
    );
    console.log(
      `  📝 Headings: ${devToolsAnalysis.uiStructure.headings.length}`,
    );
    ...
    devToolsAnalysis.uiStructure.buttons.forEach((btn, i) => {
      if (i < 5)
        console.log(
          `    "${btn.text}" (${btn.type}${btn.disabled ? ", disabled" : ""})`,
        );
    });

    console.log(
      `  📝 Input Fields: ${devToolsAnalysis.uiStructure.inputs.length}`,
    );
    devToolsAnalysis.uiStructure.inputs.forEach((input, i) => {
      if (i < 5)
        console.log(
          `    ${input.type}: ${input.placeholder || input.name || "unnamed"}`,
        );
    });
    ...
    console.log(
      `    Header: ${devToolsAnalysis.uiStructure.navigation.hasHeader ? "✅" : "❌"}`,
    );
    console.log(
      `    Navigation: ${devToolsAnalysis.uiStructure.navigation.hasNav ? "✅" : "❌"}`,
    );
    console.log(
      `    Main Content: ${devToolsAnalysis.uiStructure.navigation.hasMainContent ? "✅" : "❌"}`,
    );
    console.log(
      `    Footer: ${devToolsAnalysis.uiStructure.navigation.hasFooter ? "✅" : "❌"}`,
    );
    ...
    console.log(
      `  📊 Page Load Time: ${devToolsAnalysis.performance.totalPageLoadTime}ms`,
    );
    console.log(
      `  📊 DOM Content Loaded: ${devToolsAnalysis.performance.domContentLoaded}ms`,
    );
    console.log(
      `  📊 Resources Loaded: ${devToolsAnalysis.performance.resourcesLoaded}`,
    );
    if (devToolsAnalysis.performance.memoryUsage !== "Not available") {
      console.log(
        `  🧠 Memory Usage: ${Math.round(devToolsAnalysis.performance.memoryUsage.usedJSHeapSize / 1024 / 1024)}MB`,
      );
    }
    ...
    console.log(
      `  📍 Skip Links: ${devToolsAnalysis.accessibility.hasSkipLinks ? "✅" : "❌"}`,
    );
    console.log(
      `  🏷️  ARIA Labels: ${devToolsAnalysis.accessibility.ariaLabels}`,
    );
    console.log(
      `  🖼️  Alt Texts: ${devToolsAnalysis.accessibility.altTexts}/${devToolsAnalysis.accessibility.totalImages} images`,
    );
    if (devToolsAnalysis.accessibility.missingAltTexts > 0) {
      console.log(
        `  ⚠️  Missing Alt Texts: ${devToolsAnalysis.accessibility.missingAltTexts}`,
      );
    }
    console.log(
      `  🎯 Focusable Elements: ${devToolsAnalysis.accessibility.focusableElements}`,
    );

    // Security Report
    console.log("\n🔒 SECURITY ANALYSIS:");
    console.log(
      `  🔐 HTTPS Used: ${devToolsAnalysis.security.httpsUsed ? "✅" : "❌"}`,
    );
    console.log(
      `  🔗 Mixed Content: ${devToolsAnalysis.security.mixedContent ? "❌ Found" : "✅ None"}`,
    );
    console.log(
      `  🛡️  CSP Present: ${devToolsAnalysis.security.cspPresent ? "✅" : "❌"}`,
    );
    console.log(
      `  📜 External Scripts: ${devToolsAnalysis.security.externalScripts}`,
    );
    console.log(
      `  ⚠️  Inline Scripts: ${devToolsAnalysis.security.inlineScripts}`,
    );
    ...
    const networkAnalysis = await page.evaluate(() => {
      ...
      resources.forEach((resource) => {
        ...
        analysis.resourceTypes[extension] =
          (analysis.resourceTypes[extension] || 0) + 1;
      });
    ...
    console.log(
      `  📊 Total Network Requests: ${networkAnalysis.totalRequests}`,
    );
    ...
    if (networkAnalysis.largestResources.length > 0) {
      ...
      networkAnalysis.largestResources.forEach((resource) => {
        console.log(
          `    📦 ${resource.name} - ${resource.size} (${resource.duration})`,
        );
      });
    }
    ...
    if (isLoginPage) {
      ...
      await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().includes("cognito") && response.status() === 200,
          { timeout: 60000 },
        ),
      ...
      if (dashboardLoaded) {
        ...
        await page
          .waitForSelector("text=Projects for", { timeout: 10000 })
          .catch(() => console.log("⚠️ Projects section not found"));

        // Count projects
        const projectCount = await page.$$eval(
          "table tbody tr",
          (rows) => rows.length,
        );
    ...
    const finalAnalysis = await page.evaluate(() => {
      ...
      if (
        navigationTiming &&
        navigationTiming.loadEventEnd - navigationTiming.navigationStart > 5000
      ) {
      ...
      const errors = performance
        .getEntries()
        .filter(
          (entry) => entry.entryType === "resource" && entry.responseEnd === 0,
        );
      ...
      if (!window.awsmobile) {
        finalState.pageHealth.criticalIssues.push(
          "AWS Amplify configuration missing",
        );
      }
    ...
    console.log("\n📊 PAGE HEALTH STATUS:");
    console.log(
      `  Overall Status: ${finalAnalysis.pageHealth.hasErrors ? "❌ ISSUES FOUND" : "✅ HEALTHY"}`,
    );
    ...
    console.log("\n🎨 UI INTEGRITY STATUS:");
    console.log(
      `  Essential Elements: ${finalAnalysis.uiIntegrity.essentialElementsPresent ? "✅ PRESENT" : "❌ MISSING"}`,
    );
    ...
    console.log("\n⚡ PERFORMANCE STATUS:");
    console.log(
      `  Load Time: ${finalAnalysis.performanceHealth.loadTimeAcceptable ? "✅ ACCEPTABLE" : "❌ SLOW"}`,
    );
    console.log(
      `  Memory Usage: ${finalAnalysis.performanceHealth.memoryUsageNormal ? "✅ NORMAL" : "⚠️ HIGH"}`,
    );

    console.log("\n🔒 SECURITY STATUS:");
    console.log(
      `  HTTPS: ${finalAnalysis.securityStatus.httpsEnabled ? "✅ ENABLED" : "❌ DISABLED"}`,
    );
    console.log(
      `  Mixed Content: ${finalAnalysis.securityStatus.noMixedContent ? "✅ CLEAN" : "❌ DETECTED"}`,
    );

    // Overall grade
    const issueCount =
      finalAnalysis.pageHealth.criticalIssues.length +
    ...
    const criticalComponentsCheck = await page.evaluate(() => {
      ...
      const consoleOutput = window.performance
    ...
    console.log(
      `  - Failed requests: ${finalNetworkCheck.failedRequests.length}`,
    );
    console.log(
      `  - API fetch errors: ${finalNetworkCheck.hasFetchErrors ? "YES" : "NO"}`,
    );

    console.log("\n🔍 Critical components check:");
    console.log(
      `  - AWS Exports loaded: ${criticalComponentsCheck.awsExportsFound ? "✅ YES" : "❌ NO"}`,
    );
    console.log(
      `  - Auth components found: ${criticalComponentsCheck.authComponentsFound ? "✅ YES" : "❌ NO"}`,
    );
    console.log(
      `  - Critical functions found: ${
        criticalComponentsCheck.criticalFunctions.length > 0
    ...
    const allComponentsPresent =
      criticalComponentsCheck.awsExportsFound &&
    ...
    if (allComponentsPresent) {
      console.log(
        "✅ PASS: All critical components are present in the deployment",
      );
    } else {
      console.log(
        "❌ FAIL: Some critical components are missing from the deployment",
      );
      console.log(
        "   Run the rebuild-and-deploy-complete.sh script to fix the issues",
      );
    }
