...
async function authenticate() {
  ...
  try {
    ...
    console.log(
      "🔐 Step 1: Initiating authentication with Cognito User Pool...",
    );
    const initiateAuthResponse = await fetch(
    ...
    const identityResponse = await fetch(
      `https://cognito-identity.${CONFIG.cognito.region}.amazonaws.com/`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/x-amz-json-1.1",
        ...
        body: JSON.stringify({
          ...
          Logins: {
            [`cognito-idp.${CONFIG.cognito.region}.amazonaws.com/${CONFIG.cognito.userPoolId}`]:
              idToken,
          },
        }),
      },
    ...
    if (!identityResponse.ok) {
      ...
      console.log(
        "⚠️ Continuing with User Pool token only, DynamoDB access might fail",
      );
      return true;
    }
    ...
    console.log(
      "🎉 Full authentication successful! Both User Pool and Identity Pool credentials obtained.",
    );
    return true;
  } catch (error) {
...
const tests = {
  ...
  testHealthEndpoint: async function () {
    ...
    const response = await fetch(
      `${CONFIG.api.baseUrl}${CONFIG.api.healthEndpoint}`,
    );
  ...
  testProjectsFetch: async function () {
    ...
    try {
      ...
      const response = await fetch(
        `${CONFIG.api.baseUrl}${CONFIG.api.projectEndpoint}/${projectId}`,
        {
          headers: {
  ...
  testGenerateActa: async function () {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `❌ Failed to generate ACTA: ${response.status} ${response.statusText}`,
        );
        return false;
      }
  ...
  testDownloadPDF: async function () {
    ...
    try {
      ...
      const response = await fetch(
        `${CONFIG.api.baseUrl}${CONFIG.api.documentCheckEndpoint}/${projectId}`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        console.error(
          `❌ Document check failed: ${response.status} ${response.statusText}`,
        );
        return false;
      }
      ...
      const downloadResponse = await fetch(
        `${CONFIG.api.baseUrl}${CONFIG.api.downloadEndpoint}/${projectId}?format=pdf`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
      ...
      if (!downloadResponse.ok) {
        console.error(
          `❌ Download URL generation failed: ${downloadResponse.status} ${downloadResponse.statusText}`,
        );
        return false;
      }
      ...
      console.log(
        ...
        downloadData.url ? "URL successfully generated" : "No URL returned",
      );
  ...
  testDownloadDOCX: async function () {
    ...
    try {
      ...
      // Then get download URL
      const downloadResponse = await fetch(
        `${CONFIG.api.baseUrl}${CONFIG.api.downloadEndpoint}/${projectId}?format=docx`,
        {
          headers: {
            Authorization: idToken,
            "Content-Type": "application/json",
  ...
  testSendApproval: async function () {
    ...
    try {
      const response = await fetch(
        `${CONFIG.api.baseUrl}${CONFIG.api.approvalEndpoint}`,
        {
          method: "POST",
          headers: {
            Authorization: idToken,
      ...
      if (!response.ok) {
        console.error(
          `❌ Send approval failed: ${response.status} ${response.statusText}`,
        );
        return false;
      }
...
async function runAllTests() {
  console.log(
    "🧪 Starting end-to-end button tests with full authentication...",
  );
  console.log("-------------------------------------");
  ...
  Object.entries(results).forEach(([test, passed]) => {
    console.log(
      `${passed ? "✅" : "❌"} ${test.toUpperCase()}: ${passed ? "PASSED" : "FAILED"}`,
    );
  });
  ...
  if (!allPassed) {
    ...
    if (!results.health) {
      console.log(
        "❌ API Health endpoint failed - check if the API Gateway is accessible",
      );
    }
    ...
    if (!results.projects) {
      ...
      console.log(
        "  - User Pool authentication may not be configured correctly",
      );
      console.log(
        "  - API Gateway may not be accepting the authorization token",
      );
    ...
    if (results.projects && !results.generate) {
      ...
      console.log(
        "  - Identity Pool credentials may not be configured correctly",
      );
      console.log(
        "  - Lambda function might be failing to process the request",
      );
