...
if (dashboardInfo.exists) {
  console.log(
    `✅ Dashboard exists: ${dashboardInfo.lines} lines, ${dashboardInfo.imports} imports`,
  );
  ...
  const importMatches =
    dashboardContent.match(/import\s+.*?from\s+['"]([^'"]+)['"]/g) || [];
  ...
  const features = [
    ...
    {
      name: "API functions",
      check: /(generateActaDocument|getS3DownloadUrl|sendApprovalEmail)/g,
    },
  ];
...
Object.keys(expectedComponents).forEach((category) => {
  ...
  Object.keys(expectedComponents[category]).forEach((filePath) => {
    ...
    if (info.exists) {
      ...
      if (
        category.includes("Dashboard Core") ||
        category.includes("Authentication") ||
        category.includes("API Layer")
      ) {
...
const dynamoChecks = [
  ...
  },
  {
    ...
    patterns: [
      ...
      {
        name: "Handles project selection",
        regex: /handleProjectSelect|onProjectSelect/g,
      },
  ...
  },
  {
    ...
    patterns: [
      {
        name: "getProjectsByPM function",
        regex: /function getProjectsByPM|getProjectsByPM.*=/g,
      },
...
const dashboardAlignmentChecks = [
  {
    name: "Uses unified DynamoDB view (not separate tables)",
    pattern: /DynamoProjectsView/g,
  },
  {
    name: "Project selection functionality",
    pattern: /handleProjectSelect|onProjectSelect/g,
  },
  { name: "ACTA generation workflow", pattern: /generateActaDocument/g },
  { name: "PDF preview capability", pattern: /PDFPreview/g },
  {
    name: "Email approval system",
    pattern: /sendApprovalEmail|EmailInputDialog/g,
  },
  {
    name: "Document download (PDF/DOCX)",
    pattern: /getS3DownloadUrl.*pdf|getS3DownloadUrl.*docx/gi,
  },
  { name: "Cognito authentication flow", pattern: /getCurrentUser|useAuth/g },
  {
    name: "Loading states management",
    pattern: /actionLoading|setActionLoading/g,
  },
...
if (fs.existsSync("src/pages/Dashboard.tsx")) {
  ...
  const antiPatterns = [
    ...
    {
      name: "Manual project entry forms",
      pattern: /manual.*entry|entry.*manual/gi,
    },
  ...
  antiPatterns.forEach((check) => {
    ...
    const status = count === 0 ? "✅" : "❌";
    console.log(
      `   ${status} ${check.name}: ${count} occurrences (should be 0)`,
    );
  });
}
...
console.log(
  `Routing configuration: ${routingReady ? "✅ Ready" : "❌ Issues"}`,
);
...
if (dashboardInfo.lines > 300) {
  console.log(
    "⚠️  Dashboard is still quite large - consider further modularization",
  );
}
