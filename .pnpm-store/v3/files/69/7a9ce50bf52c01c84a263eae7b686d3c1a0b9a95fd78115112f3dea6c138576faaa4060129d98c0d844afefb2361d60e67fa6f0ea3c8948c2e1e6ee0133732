  ...
  return {
    ...
    getActionTriggerProps(props2) {
      ...
      return normalize.button({
        ...
        hidden: utils.match(props2.action, {
...
var machine = core.createMachine({
  ...
  computed: {
    ...
    progressPercent: ({ context, prop }) => {
      ...
      if (prop("countdown")) {
        return utils.clampValue(toPercent(currentMs, targetMs, startMs), 0, 1);
      }
      return utils.clampValue(toPercent(currentMs, startMs, targetMs), 0, 1);
    }
  },
  states: {
    idle: {
  ...
  implementations: {
    effects: {
      keepTicking({ prop, send }) {
        return utils.setRafInterval(({ deltaMs }) => {
